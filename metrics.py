...
def block_view(A, block=(3, 3)):
    ...
    shape = np.asarray((A.shape[0]/ block[0], A.shape[1]/ block[1])+ block).astype(np.uint)
    strides = np.asarray((block[0]* A.strides[0], block[1]* A.strides[1])+ A.strides).astype(np.uint)
